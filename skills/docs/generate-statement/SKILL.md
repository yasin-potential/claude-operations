...
**Shares the same sequence with generate-invoice.** Read `invoices/invoice-records.json` to determine the next available number. See Step 5.3 for auto-increment logic.
...
**Directory**: `invoices/`
**Filename**: `[거래명세서] {ClientName} - {StatementType}.html`
...
**PDF filename**: `[거래명세서] {ClientName} - {StatementType}.pdf`
...
After generating the HTML and PDF, save the metadata to `invoices/invoice-records.json`.
...
```json
{
  ...
  "date": "YYYY-MM-DD",
  "files": {
    "html": "invoices/[거래명세서] ClientName - Type.html",
    "pdf": "invoices/[거래명세서] ClientName - Type.pdf"
  }
}
```
...
HTML: invoices/[거래명세서] {ClientName} - {StatementType}.html
PDF:  invoices/[거래명세서] {ClientName} - {StatementType}.pdf
...
-> Output: `invoices/[거래명세서] 오누이 - 프로젝트.pdf`
...
-> Output: `invoices/[거래명세서] 테스트회사 - 월결제.pdf`
...
-> Output: `invoices/[거래명세서] Global Corp - Statement.pdf`
