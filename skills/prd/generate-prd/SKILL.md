---
name: generate-prd
description: "Generate a Feature PRD (Phase A) from client input. Focuses on feature definitions, user flows, and page-level specs. Technical sections (Schema, Permissions) are generated separately via /generate-tech-prd (Phase B)."
argument-hint: "Path to client answer file (e.g., /path/to/client-answers.md)"
---

# Generate Feature PRD — Phase A

> **Architecture Note**: This skill generates Feature PRDs (Phase A) only.
> Technical sections (Schema, Permissions, System Design) are generated separately via `/generate-tech-prd` (Phase B).
> Always run Phase A first, then Phase B.
>
> **Terminology**: "Phase A/B" = skill-level split (Feature vs Technical PRD). "Phase 1-4" = internal pipeline stages within this skill.

Generate a Feature PRD from client input using a 4-Phase pipeline with 5 agents.
This skill produces Sections 0-4 + 2.5 (features, flows, pages, notifications).
Technical sections (Schema, Permissions, System Design) are generated separately via `/generate-tech-prd` (Phase B).
...
Pre-intake Form (클라이언트 작성, 5분)
  ...
  → Phase 1~4 자동 진행 (PM 확인 요청 3번)
  → 최종 Feature PRD + Client Checklist 출력
```
...
| File | Description |
|------|-------------|
| `{AppName}_FeaturePRD_{YYMMDD}.md` | 최종 Feature PRD |
...
3. `/generate-tech-prd` 실행 → Technical PRD (Schema, Permissions, System Design)
...
| **support** | sonnet | Fix QA FAIL items + apply PM review feedback (max 3 rounds) |

## Reference Files

All references located under `skills/prd/generate-prd/references/`:
...
2. Create folders:
   ...
   ```
3. Copy `skills/prd/generate-prd/templates/interview-presenter.html` (relative to skill directory) → `.claude-project/prd/{ProjectName}/interview-live.html`
4. Display to PM:
   ```
...
**Input mode routing:**
```
$ARGUMENTS contains file path    → File mode: validate file → continue to 1.2
$ARGUMENTS contains "--interview" → Interactive mode: jump to "Interactive Interview Mode" section
                                    (returns compiled client-answers.md, then continue to 1.2)
$ARGUMENTS is empty              → Ask user which mode via AskUserQuestion:
                                    1. 파일 입력 (답변 파일 경로를 알려주세요)
                                    2. 인터뷰 모드 (클라이언트와 실시간 인터뷰)
```
...
These flags are used by Phase B (`/generate-tech-prd`) to activate conditional sections.
...
Filename: `[AppName]_FeaturePRD_[YYMMDD].md`
...
├── drafts/
└── {AppName}_FeaturePRD_{YYMMDD}.md  ← Final Feature PRD
...
4. **Run `/generate-tech-prd` to generate Technical PRD (Schema, Permissions, System Design)**
5. Update PRD with client responses using `/generate-prd [answer-file]`
```
