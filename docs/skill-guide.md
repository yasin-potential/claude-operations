...
## A. PRD Lifecycle (6 skills + 1 agent)

### Pipeline Flow

```
Client answers ──→ /generate-prd (Phase A: Feature PRD)
                       ↓
                   /generate-tech-prd (Phase B: Technical PRD)
                       ↓
                   Development begins
...
4. Run `/generate-prd` → Feature PRD + client checklist
...
8. Run `/generate-tech-prd` for developer handoff
9. Run `/generate-korean-prd` for client deliverable

### Skill Details

| Skill | Command | Prerequisites | Output |
|-------|---------|---------------|--------|
| **generate-prd** | `/generate-prd <input-file>` | Client answer file | Feature PRD + client checklist |
| **generate-tech-prd** | `/generate-tech-prd [prd-file]` | Completed Feature PRD with NO unresolved `[💡 Recommended]` markers | Complete PRD (Sections 5-7: Tech Stack, Data Model, Permissions) |
...
**Notes:**
- `generate-tech-prd` will ABORT if unresolved `[💡 Recommended]` items remain — resolve them first via `/update-prd`
- `input-classifier` runs automatically before `/update-prd` — never invoke it directly
- `generate-prd` uses 5 agents internally (parser, feature-writer, checklist, qa-feature, support)
...
## B. Document Generation (6 skills)

| Skill | Command | Prerequisites | Output | Language |
|-------|---------|---------------|--------|----------|
| **generate-ppt** | Step 1: `/generate-ppt "Topic"` → .md outline<br>Step 2: `/generate-ppt presentations/topic.md` → .html | Topic string or markdown file | Reveal.js HTML presentation | English |
| **generate-sop** | `/generate-sop "Topic"` | `NOTION_API_KEY` env var | Notion page in Team SOP database | EN or KR (user selects) |
| **generate-invoice** | `/generate-invoice [--client "Name"] [--no-vat]` | Client info; Chrome for PDF | HTML + PDF + JSON record in `invoices/` | Korean (견적서) |
...
| **review-command** | `/review-command <skill-file-path>` | Skill .md file to validate | Score (0-100) + recommendations | English |

**Notes:**
- `generate-ppt` is a two-step process: first generate the .md outline, review/edit it, then generate the .html
...
## E. Workflow & Development (4 skills)

| Skill | Command | When to Use | Output |
|-------|---------|-------------|--------|
| **deployment** | *(integrated in fullstack pipeline, Phase 9)* | After QA passes (95%+ pass rate) | Deployments to dev/staging/production with rollback procedures |
| **iteration-manager** | `/iteration-manager --new-iteration` or `--iterate <phase>` | Re-running pipeline phases after changes | Iteration tracking, phase cascade marking |
| **create-dev-pr** | *(auto-triggered on "fix completed", "feature done", "ready for PR")* | After finishing a bug fix or feature | PR to dev branch with conventional commit title |
| **code-cleanup** | `/code-cleanup [--fix] [--category imports\|exports\|functions\|files\|packages\|columns\|duplicates]` | Before audits, during refactoring | Dead code scan report, optional auto-removal |

**Notes:**
- `deployment` environment progression: Development (Dokploy auto) → Staging (Dokploy manual) → Production (AWS ECS)
- `iteration-manager` tracks phase cascades — changing PRD marks all downstream phases as pending
- `create-dev-pr` auto-generates PR body with Summary, Type, Changes Made, Test Plan
- `code-cleanup` is universal: auto-detects TypeScript, JavaScript, Python, Go, Rust, Java, PHP
...
```
PRD Pipeline:
  /generate-prd ──→ /generate-tech-prd ──→ Development
        ↕
  /update-prd (with /input-classifier auto-trigger)
        ↓
...
6. **Review markdown before final generation** — `/generate-ppt` and `/generate-prd` produce intermediate files for review
